...
export const reviewApi = baseApi.injectEndpoints({
    endpoints: (builder) => ({
        ...
        // Public: submit a product review. Unlike `publicCreateReview` this does
        // no optimistic insert — the review is pending until an admin approves it,
        // so showing it in the list straight away would be a lie.
        submitProductReview: builder.mutation({
            ...
            invalidatesTags: (result, error, arg: any) => [{ type: 'Reviews' as const, id: arg.product }],
        }),
        ...
        // Public: approved reviews for the homepage strip — GET /api/reviews/showcase
        getShowcaseReviews: builder.query({
            query: (limit = 12) => `/reviews/showcase?limit=${limit}`,
            providesTags: ['Reviews'],
        }),
        ...
        getProductReviews: builder.query({
            ...
            // `any` so callers can pass paging/sort params alongside productId
            providesTags: (result: any, error: any, { productId }: { productId: string;[k: string]: any }) => [{ type: 'Reviews' as const, id: productId }]
        }),
